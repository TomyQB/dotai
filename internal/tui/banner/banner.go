// Package banner renders the mem-cli ASCII banner. It has no dependencies on
...
const tagline = "living agent memory, enforced"
...
// wordmark is the "mem-cli" ANSI Shadow figlet. 6 lines, 49 cols wide.
const wordmark = `███╗   ███╗███████╗███╗   ███╗       ██████╗██╗     ██╗
████╗ ████║██╔════╝████╗ ████║      ██╔════╝██║     ██║
██╔████╔██║█████╗  ██╔████╔██║█████╗██║     ██║     ██║
██║╚██╔╝██║██╔══╝  ██║╚██╔╝██║╚════╝██║     ██║     ██║
...
// wordmarkSmall is a compact box-drawing version that fits in ~24 chars.
const wordmarkSmall = ` ┏┳┓┏━╸┏┳┓   ┏━╸╻  ╻
 ┃┃┃┣╸ ┃┃┃╺━╸┃  ┃  ┃
 ╹ ╹┗━╸╹ ╹   ┗━╸┗━╸╹`
...
//  3. otherwise:                 compact title "mem-cli v0.1.0"
...
// "mem-cli · <screen>". Width is used only for centering.
...
func renderCompact(width int) string {
	return center(styles.Title.Render("mem-cli v"+version.Version), width)
}
