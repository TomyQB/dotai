// Package localuninstall implements the Uninstall screen for `dotai local`.
// The screen mirrors localinstall: a checklist of profiles, confirming removes
// the skills (and CLAUDE.md) each profile owns from targetDir/.claude/.
package localuninstall
...
// doneMsg is emitted once uninstall work finishes (or fails).
type doneMsg struct {
	successes []string
	err       error
}

// Model is the Bubble Tea model for the local Uninstall screen.
type Model struct {
	targetDir     string
	rows          []profileRow
	cursor        int
...
// New returns an Uninstall screen bound to targetDir. Rows corresponding to
// profiles that are not installed are shown disabled (unselectable, greyed).
func New(targetDir string) Model {
	profiles := localprofile.All()
	rows := make([]profileRow, len(profiles))
	for i, p := range profiles {
		rows[i] = profileRow{profile: p, installed: p.IsInstalled(targetDir)}
	}
	return Model{targetDir: targetDir, rows: rows}
}

// Title satisfies messages.Screen.
func (m Model) Title() string { return "local uninstall" }

// Init has no startup work.
func (m Model) Init() tea.Cmd { return nil }

// Update handles key events and uninstall-completion messages.
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case doneMsg:
		m.phase = phaseDone
		if msg.err != nil {
			m.toast = "uninstall failed: " + msg.err.Error()
			m.toastErr = true
		} else if len(msg.successes) == 0 {
			m.toast = "no profiles selected"
		} else {
			m.toast = "removed: " + strings.Join(msg.successes, ", ")
		}
		for i := range m.rows {
			m.rows[i].installed = m.rows[i].profile.IsInstalled(m.targetDir)
...
func (m Model) handleKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case " ":
		if len(m.rows) > 0 && m.rows[m.cursor].installed {
	...
	case "enter":
		...
		return m, runUninstall(m.targetDir, m.selectedProfiles())
	case "esc", "left", "q":
		return m, popCmd()
	}
	return m, nil
...
func (m Model) View() string {
	...
	header := banner.RenderCompact(innerWidth, "local · uninstall")

	var body strings.Builder
	body.WriteString(styles.Help.Render("target: "+m.targetDir) + "\n\n")
	body.WriteString(styles.SectionTitle.Render("Pick profiles to remove:"))
	body.WriteString("\n\n")
	...
	if m.phase == phaseRunning {
		body.WriteString("\n" + styles.WarningText.Render("Uninstalling...") + "\n")
	}
...
func (m Model) Footer() string {
	...
	return styles.FooterHints("↑/↓", "move", "space", "toggle", "enter", "remove", "esc", "back")
}

// runUninstall removes every selected profile from targetDir.
func runUninstall(targetDir string, profiles []localprofile.Profile) tea.Cmd {
	return func() tea.Msg {
		var successes []string
		for _, p := range profiles {
			if err := p.Uninstall(targetDir); err != nil {
