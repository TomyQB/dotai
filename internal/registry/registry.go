...
// Entry is a single project registered with memcli.
type Entry struct {
...
// Error wraps a registry read/parse failure with context about the offending
// path. Callers can use errors.As to unwrap.
type Error struct {
	Path string
	Err  error
}
...
// an empty v2 registry. Malformed JSON returns an *Error.
